...
async def view_editor(callback: types.CallbackQuery):
    ...
    for pair in db["pairs"]:
        ...
        keyword = pair.get("keyword_filter")
        remove = pair.get("remove_word")

        text += f"<b>{name}</b>\n"

        if keyword:
            text += f"Keyword : {keyword}\n"
            text += f"Remove  : {remove}\n\n"
        else:
            text += "No Editor\n\n"
...
async def delete_editor(callback: types.CallbackQuery):
    ...
    db["pairs"][index]["keyword_filter"] = None
    db["pairs"][index]["remove_word"] = None
...
# ================= ADD FILTER =================
    ...
    if state.get("step") == "set_keyword":
        ...
        data["pairs"][index]["keyword_filter"] = text
    
        await save_db(data)   # ⭐ यह add करो
    ...
    if state.get("step") == "set_remove":
        ...
        data["pairs"][index]["remove_word"] = text
...
async def forward_handler(event):
    ...
    for pair in db.get("pairs", []):
        ...
        keyword = pair.get("keyword_filter")
        remove_word = pair.get("remove_word")
