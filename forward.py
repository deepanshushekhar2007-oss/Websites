...
api_id = 34958210  # <-- Yaha apna API_ID dalo
api_hash = "6923cd2c34591c8e26b30ade39c7518b"  # <-- Yaha apna API_HASH dalo
...
if not MONGO_URI:
    raise ValueError("MONGO_URI not found! Set it in Render Environment Variables.")
...
async def forward_handler(event):
    ...
    for pair in db.get("pairs", []):
        ...
        try:
            ...
            # ⭐ MESSAGE COPY (best method)
            sent = await client.copy_message(
                pair["to"],      # destination chat
                event.chat_id,   # source chat
                event.message.id,
                buttons=buttons
            )
