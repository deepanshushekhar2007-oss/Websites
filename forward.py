...
async def start_cmd(message: types.Message):
    ...
    if await is_admin(uid):
        ...
        await message.answer(
            ...
            "Use <code>/panel</code> in your Telegram account to open the control panel.",
            parse_mode="html"
        )
        return
...
@userbot.on(events.NewMessage(pattern="/panel"))
...
async def remove_pair(event):
    ...
    if index >= len(data["pairs"]):
        await event.answer("❌ Invalid Pair", alert=True)
        return
...
async def toggle_pair_menu(event):
    ...
    if not data["pairs"]:
        await event.answer("❌ No pairs available", alert=True)
        return
...
async def remove_button_menu(event):
    ...
    if not data["buttons"]:
        await event.answer("❌ No buttons available", alert=True)
        return
...
async def private_handler(event):
    ...
    if state.get("step") == "add_pair_from":
        ...
        try:
            ...
            await event.respond("❌ Send valid SOURCE Chat ID")
            return
    ...
    if state.get("step") == "add_pair_to":
        ...
        try:
            ...
            await event.respond("❌ Send valid DESTINATION Chat ID")
            return
    ...
    if state.get("step") == "rename_pair":
        ...
        if index >= len(data["pairs"]):
            await event.respond("❌ Invalid Pair")
            user_states.pop(uid)
            return
