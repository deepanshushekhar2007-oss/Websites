...
# ================= OTP FORMAT CLEANER =================
        ...
        # OTP text में check
                otp_match = re.search(r"\d{3,6}[- ]?\d{0,3}", text)

                if otp_match and "-" in otp_match.group():
                    otp_line = f"Otp - {otp_match.group()}"

        # अगर text में नहीं मिला तो button check
                if not otp_line and event.message.buttons:
                    ...
                    for row in event.message.buttons:
                        for btn in row:
                            ...
                            otp_match = re.search(r"\d{3,6}[- ]?\d{0,3}", btn_text)
