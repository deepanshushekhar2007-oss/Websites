...
async def pair_creator(message: Message):
...
async def private_handler(message: Message):
    ...
    # If user has no active state, ignore
    if uid not in user_states:
        return
    ...
    if state.get("step") == "add_button_text":
        # Save the button text and ask for URL
        user_states[uid] = {"step": "add_button_url", "text": message.text}
